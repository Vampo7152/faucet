...
const noFunds = new MessageEmbed({
	description: "Low on funds! Consider donating some SOL or please check later!",
	color: "#DE1738"
});
...
export abstract class SlashExample {
	...
	async faucet(
		...
		if (network === "mainnet-beta") {
			...
			await interaction.editReply("Mainnet Faucet? maybe ask @kashdhanda on twitter <:PeepoLove:937430087828115476>");
			return;
		}
		try {
			// get balance
			...
			if (bal.balance > RICHBAL) {
				...
				await interaction.editReply(`You have more than enough funds, consider donating some [here!](<https://solscan.io/address/${PAYER}>) <:salute:975670283170119691>`); // interaction.editReply({ embeds: [richEmbed] });
				return;
			}
		} catch (error:any) {
